...
\begin{exercise}
	...
	(b) show that every extreme point of $\mathcal{Q}$ is an impulse.%
		...
		Prove the contra-positive: fix an arbitrary $q \in \mathcal{Q}$, and try to show that it isn't an extreme point, by constructing distinct $q^-,q^+ \in \mathcal{Q}$ such that $q = \alpha q^- + (1-\alpha) q^+$ for some $\alpha \in (0,1)$.
		...
		% A solution: $q^- \coloneqq q + \frac{1}{2}(1-q)$, $q^+ \coloneqq q + \frac{1}{2}(1-q)$ and $\alpha=1/2$.}
	%
\end{exercise}
...
More formally, the claim is that the good sells a.s. in finite time,
at a price $p^\Delta$ that vanishes as $\Delta \to 0$.
The classical intuition for this conjecture is that when the period length $\Delta$ is small, the monopolist in period $n$
engages in near-perfect competition with her period-$(n+1)$ self,
driving the price down to marginal cost (which we've normalised to zero).
...
Let's assume that the agent's valuation $T$ is supported on an interval $S \subseteq [0,1]$, and recall that we normalise the monopolist's valuation of the good to zero.
(The results below also require some further technical assumptions.)
