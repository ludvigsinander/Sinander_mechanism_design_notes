...
\date{\small This version: 5 February 2026}
...
\hspace{1pt}\vfill
\noindent
Copyright \copyright{} 2026 Carl Martin Ludvig Sinander.
...
A \emph{measure} on a measurable space $(X,\mathcal{X})$ is a map $\mu : \mathcal{X} \to [0,\infty]$ that is normalised and countably additive: $\mu(\varnothing)=0$ and for any countable collection $A_1,A_2,\dots$ of pairwise disjoint measurable subsets of $X$,
...
while still giving the same result as the Riemann integral whenever the latter exists, provided we are integrating over a bounded interval.
...
then $\phi$ is called \emph{affine.}
...
If $\phi$ is affine (not merely convex), then you don't have to worry about the `suitably continuous' bit, as affineness gives it to you for free.
